...
if(isset($_REQUEST['action']) && !empty($_REQUEST['action'])){ 
    if($_REQUEST['action'] == 'addToCart' && !empty($_REQUEST['id'])){ 
        ...
        $sqlQ = "SELECT * FROM product WHERE id=?"; 
        ...
        $redirectURL = $insertItem?'viewCart.php?id='. $id:'menu.php'; 
        ...
        // Redirect to cart page 
        $redirectURL = 'viewCart.php?id='. $id; 
        ...
        if(empty($errorMsg)){ 
            ...
            $sqlQ = "INSERT INTO customers_orderdetails (first_name,last_name,email,phone,address,created,modified) VALUES (?,?,?,?,?,NOW(),NOW())"; 
            $stmt = $db->prepare($sqlQ); 
            ...
            if($insertCust){ 
                ...
                if($insertOrder){ 
                    ...
                    if(!empty($cartItems)){ 
                        ...
                        $redirectURL = 'orderSuccess.php?id='.base64_encode($orderID).'&p_id='. $id; 
                    }else{ 
