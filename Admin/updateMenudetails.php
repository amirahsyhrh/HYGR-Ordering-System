...
if(isset($_POST["submit"])){ 
    ...
    if(!empty($_FILES["image"]["name"])) { 
        ...
        if(in_array($fileType, $allowTypes)){ 
            ...
            $insert = $connect->query("update `product` set `id`='$mid',`product_name`='$name', `product_price`= '$price' , `product_category` = '$category', `product_details` = '$details', `product_image` = '$imgContent' where `id`='$mid'");
             
            if($insert){ 
                $status = 'success'; 
                //display message box Record Been Added
				print '<script>alert("Product Has Been Updated!");</script>';
